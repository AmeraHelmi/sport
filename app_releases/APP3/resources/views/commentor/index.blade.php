...
<ul class="alerts-list" style="display:none;" id="show">
  <li>
     <div class="alert alert-success alert-dismissable">
           <i class="icon-remove-sign"></i> تم أضافة معلق بنجــــــــــاح!.
...
<a class="btn btn-primary" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
		<i class="fa fa-plus-circle"  style="font-size: 18px;"></i> أضافة معلق
</a>
<div class="widget-content-white glossed">
		<div class="padded">
				<table id="Commentor" class="table table-striped table-bordered table-hover datatable">
						<thead>
								<tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i> أضافة معلق</h4>
            </div>
            <form role="form" method="POST" class="addForm" action="{{ url('/commentor/store') }}" data-toggle="validator">
                <div class="modal-body">
                    @include('commentor.form')
                </div>
...
<div class="modal fade" id="editcommentatorModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            ...
            <form role="form" id="update_form" method="POST" class="editForm" data-id="" action="{{ url('/commentor/update') }}" data-toggle="validator">
                <div class="modal-body">
                    @include('commentor.formupdate')
                </div>
                <div class="modal-footer">
...
$(document).ready(function() {
  ...
  $('#showupdate').show(100);
  $('#editcommentatorModal').modal('hide');
  oTable.ajax.reload();
  oTable.draw();
      });
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('commentor') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 $data = JSON.parse(res.data);
									 populateForm($data, document.getElementsByClassName("editForm")[0] );
	                 $('#editcommentatorModal form').attr("data-id", self.data('id') );
	                 $('#editcommentatorModal').modal('show');
	             },
	             error: function(){
	                 self.button('reset');
	                   $('.alerts-list').append(
