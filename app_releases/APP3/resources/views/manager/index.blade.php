...
<div class="modal fade" id="editmanagementModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
  ...
  $.ajax({
	...
	oTable.draw();
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('manager') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 $data = JSON.parse(res.data);
									 populateForm($data, document.getElementsByClassName("editForm")[0] );
	                 $('#editmanagementModal form').attr("data-id", self.data('id') );
	                 $('#editmanagementModal').modal('show');
	             },
	             error: function(){
	                 self.button('reset');
	                 $('.alerts-list').append(
                    ...
                    {data: 'cityname', name: 'cityname'},
										...
										{data: 'selection_type', name: 'selection_type'},
