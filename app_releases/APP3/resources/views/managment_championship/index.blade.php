...
<div class="modal fade" id="editgroupModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    ...
    $("#addModal form").on('submit', function(e){
        if (!e.isDefaultPrevented())
        {
            var self = $(this);
            $.ajax({
                url: '{!!URL::route('addmanagment_championship')!!}',
                type: "POST",
                data: self.serialize(),
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('managment_championship') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
             ...
             $("#editgroupModal form").validator().on('submit', function(e){
                 if (!e.isDefaultPrevented())
                 {
                     var self = $(this);
                     $.ajax({
                         url: "{{ url('managment_championship') }}" + "/" +  self.attr("data-id"),
                         type: "POST",
