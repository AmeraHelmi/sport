	 ...
	 <div class="container-fluid">
             ...
             <br>
                ...
                <ul class="alerts-list" style="display:none;" id="show">
                    <li>
                        <div class="alert alert-success alert-dismissable">
                            <i class="icon-remove-sign"></i> تم أضافة نادى بنجـــــــــــاح!.
                ...
                <ul class="alerts-list" style="display:none;" id="showupdate">
                    <li>
                        <div class="alert alert-success alert-dismissable">
                            <i class="icon-remove-sign"></i> تم تحديث البيانات نادى بنجـــــــــــاح!.
                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                        </div>
                    </li>
                </ul>
                <a class="btn btn-primary" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
		                <i class="fa fa-plus-circle"  style="font-size: 18px;"></i> أضافة نادى
                </a>
                <a class="btn btn-primary add" data-toggle="modal" data-target="#best_team" style="margin-bottom:20px;" >
                    <i class="glyphicon glyphicon-arrow-left"></i>  أاضافة افضل نادى
                </a>
                <div class="widget-content-white glossed">
                      ...
                      <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
                          <div class="modal-dialog">
                              <div class="modal-content">
                                  <div class="modal-header">
                                      ...
                                      <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i> أضافة نادى</h4>
                                  </div>
                                  <form role="form" method="POST" class="addForm" action="{{ url('/team/store') }}" data-toggle="validator" enctype="multipart/form-data" >
                        ...
                        <!-- افضل نادى -->
                        <div class="modal fade" id="best_team" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true" >
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                        <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i>تفعيل افضل نادى</h4>

                                            <form role="form" method="POST"  class="today_form" action="{{ url('/team/best_team') }}" data-toggle="validator">
                                                <div class="modal-body">
                                                        @include('team.best_team')
                                                </div>
                                              <div class="modal-footer">
                        ...
                        <div class="modal fade" id="editplayerModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <form role="form"  method="POST" class="editForm" data-id="" action="{{ url('/team/update') }}" data-toggle="validator" enctype="multipart/form-data">
                                        <div class="modal-body">
                                            @include('team.form')
                                        </div>
                                        <div class="modal-footer">
...
$.ajax({
  ...
  $("#submitForm_today").on('click', function(e){
      $('#best_team').modal('hide');
      $('#show').show(100);

  });
  $("#best_team form").on('submit', function(e){
      if (!e.isDefaultPrevented())
      {
          var self = $(this);
          $.ajax({
              url: '{{ url('/team/best_team') }}',
              type: "POST",
              data: self.serialize(),
              success: function(res){
                  $('.best_team')[0].reset();
                  $('.alerts-list').append(
                      '<li>\
                          <div class="alert alert-success alert-dismissable">\
                              <i class="icon-check-sign"></i>تم  اضافة افضل نادى!\
                              <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
                          </div>\
                      </li>');
              },
    ...
    oTable.draw();
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('team') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
