...
<div class="content-wrapper">
		<div class="container-fluid">
            ...
            <br>
                ...
                <a class="btn btn-primary add" data-toggle="modal" data-target="#best_referee" style="margin-bottom:20px;" >
                    <i class="glyphicon glyphicon-arrow-left"></i>  أاضافة افضل حكم
                </a>
...
<!-- افضل حكم -->
                  <div class="modal fade" id="best_referee" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true" >
                      <div class="modal-dialog">
                          <div class="modal-content">
                              <div class="modal-header">
                                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                  <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i>تفعيل افضل حكم</h4>

                                      <form role="form" method="POST"  class="today_form" action="{{ url('/referee/best_referee') }}" data-toggle="validator">
                                          <div class="modal-body">
                                                  @include('referee.best_referee')
                                          </div>
                                        <div class="modal-footer">
...
<script type="text/javascript">
  ...
  $("#submitForm_today").on('click', function(e){
      $('#best_referee').modal('hide');
                      $('#show').show(100);
  });
  $("#best_referee form").on('submit', function(e){
      if (!e.isDefaultPrevented())
      {
          var self = $(this);
          $.ajax({
              url: '{{ url('/referee/best_referee') }}',
              type: "POST",
              data: self.serialize(),
              success: function(res){
                  $('.best_referee')[0].reset();

                  $('.alerts-list').append(
                      '<li>\
                          <div class="alert alert-success alert-dismissable">\
                              <i class="icon-check-sign"></i>تم  اضافة افضل  حكم!\
                              <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
                          </div>\
                      </li>');
              },
  ...
  oTable.draw();
	...
	oTable.draw();
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('referee') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 $data = JSON.parse(res.data);
									 populateForm($data, document.getElementsByClassName("editForm")[0] );
	                 $('#editrefereeModal form').attr("data-id", self.data('id') );
	                 ...
	                 $('.alerts-list').append(
	                     '<li>\
							...
							oTable = $('#players').DataTable({
								...
								"columns": [
										{data: 'refereename', name: 'refereename'},
