...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form role="form"  method="POST" class="addForm" action="{{ url('/g_album/store') }}" data-toggle="validator">
                <div class="modal-body">
                    @include('g_album.form')
                </div>
...
<div class="modal fade" id="editagentModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form role="form" id="update_form" method="POST" class="editForm" data-id="" action="{{ url('/g_album/update') }}" data-toggle="validator">
                <div class="modal-body">
                    @include('g_album.form')
                </div>
                <div class="modal-footer">
    ...
    $("#addModal form").on('submit', function(e){
        if (!e.isDefaultPrevented())
        {
		    ...
		    var self = $(this);
		         self.button('loading');
		         $.ajax({
		             url: "{{ url('g_album') }}" + "/" + self.data('id') + "/edit" ,
		             type: "GET",
		             success: function(res){
		                 self.button('reset');
		     ...
		     $("#editagentModal form").validator().on('submit', function(e){
		         if (!e.isDefaultPrevented())
		         {
		             var self = $(this);
		             $.ajax({
		                 url: "{{ url('g_album') }}" + "/" +  self.attr("data-id"),
		                 type: "POST",
