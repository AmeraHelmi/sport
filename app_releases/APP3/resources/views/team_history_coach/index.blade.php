				...
				<div class="row">
						...
						<br>
								...
								<ul class="alerts-list" style="display:none;" id="show">
  									<li>
     										<div class="alert alert-success alert-dismissable">
           									<i class="icon-remove-sign"></i> تم أضافة تاريخ مدرب بنجــــــــــاح!.
								...
								<ul class="alerts-list" style="display:none;" id="showupdate">
  									<li>
     										<div class="alert alert-success alert-dismissable">
           									<i class="icon-remove-sign"></i> تم تحديث البيانات مدرب بنجــــــــــاح!.
           									<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
       									</div>
   									</li>
								</ul>
								<a class="btn btn-primary" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
										<i class="fa fa-plus-circle"  style="font-size: 18px;"></i> أضافة تاريخ مدرب
								</a>
    									...
    									<div class="modal-dialog">
        									<div class="modal-content">
            										<div class="modal-header">
                										...
                										<h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i> أضافة تاريخ مدرب</h4>
            										</div>
            										<form role="form" method="POST" class="addForm" action="{{ url('/team_history_coach/store') }}" data-toggle="validator" enctype="multipart/form-data" >
                											<div class="modal-body">
                    											@include('team_history_coach.form')
                											</div>
										...
										<div class="modal fade" id="editgroupModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    										<div class="modal-dialog">
        										<div class="modal-content">
            										...
            										<form role="form"  method="POST" class="editForm" data-id="" action="{{ url('/team_history_coach/update') }}" data-toggle="validator" enctype="multipart/form-data">
                											<div class="modal-body">
                    												@include('team_history_coach.form_update')
                											</div>
                											<div class="modal-footer">
...
<script type="text/javascript">
	function select_team()
	{
  		...
  		$.ajax({
      		url: '{{ url('team_history_coach/get_teams') }}',
      		type: "POST",
    			data:{
        				team_type:$team_type
      			},
...
$.ajax({
    ...
    $("#addModal form").on('submit', function(e){
        if (!e.isDefaultPrevented())
        {
            var self = $(this);
            $.ajax({
                url: '{!!URL::route('addteam_history_coach')!!}',
                type: "POST",
                data: self.serialize(),
                success: function(res){
                    $('.addForm')[0].reset();
                    $('.alerts-list').append(
                        '<li>\
                            <div class="alert alert-success alert-dismissable">\
                                <i class="icon-check-sign"></i> تم  أضافة تاريخ مدرب بنجــــاح!\
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
                            </div>\
                        </li>');
                        oTable.ajax.reload();
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('team_history_coach') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
             ...
             $("#editgroupModal form").validator().on('submit', function(e){
                 if (!e.isDefaultPrevented())
                 {
                     var self = $(this);
                     $.ajax({
                         url: "{{ url('team_history_coach') }}" + "/" +  self.attr("data-id"),
                         type: "POST",
                         ...
                         success: function(res){
                             ...
                             $('.alerts-list').append(
                                 '<li>\
                                     <div class="alert alert-success alert-dismissable">\
                                         <i class="icon-check-sign"></i>تم تحديث البيانات بنجـــاح!\
                                         <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
                                     </div>\
                                 </li>');
                                 oTable.ajax.reload();
