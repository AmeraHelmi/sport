...
<script type="text/javascript">
			...
			// alert($match_id);
			$.ajax({
					url: '{{ url('reserve_player/getteams') }}',
					type: "POST",
					data:{
						match_id:$match_id
					},
		...
		$team_id=$('#team_id').val();
		$.ajax({
				url: '{{ url('reserve_player/getplayers') }}',
				type: "POST",
				data:{
					team_id:$team_id
				},
	...
	$(document).ready(function() {
		...
		$("#addModal form").on('submit', function(e){
				if (!e.isDefaultPrevented())
				{
						var self = $(this);
						$.ajax({
								url: '{!!URL::route('addreserve_player')!!}',
								type: "POST",
								data: self.serialize(),
								success: function(res){
										$('.addForm')[0].reset();
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('reserve_player') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 ...
	                 $('.alerts-list').append(
		 		     ...
		 		     $("#editplayersteamModal form").validator().on('submit', function(e){
		 		         if (!e.isDefaultPrevented())
		 		         {
		 		             var self = $(this);
		 		             $.ajax({
		 		                 url: "{{ url('reserve_player') }}" + "/" +  self.attr("data-id"),
		 		                 type: "POST",
